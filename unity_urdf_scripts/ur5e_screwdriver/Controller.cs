...
namespace RosSharp.Control
{
    ...
    public class Controller : MonoBehaviour
    {
        ...
        // Add a joint_index for each joint 
        public float shoulder_target_theta = 0;
        public float upperarm_target_theta = 0;
        public float forearm_target_theta = 0;
        public float wrist1_target_theta = 0;
        public float wrist2_target_theta = 0;
        public float wrist3_target_theta = 0;
        public float screwdriver_target_offset = 0;

        [Tooltip("Color to highlight the currently selected join")]
        public Color highLightColor = new Color(255, 0, 0, 255);
        public bool enable_joint_on_gui = false;
        ...
        void Update()
        {
            ...
            if (enable_joint_on_gui){
                UpdateJointTheta(1, shoulder_target_theta);
                UpdateJointTheta(2, upperarm_target_theta);
                UpdateJointTheta(3, forearm_target_theta);
                UpdateJointTheta(4, wrist1_target_theta);
                UpdateJointTheta(5, wrist2_target_theta);
                UpdateJointTheta(6, wrist3_target_theta);
                UpdateJointTheta(8, screwdriver_target_offset);
            }
            


            // Debug.Log("Twist: ");
            // var twist = articulationChain[selectedIndex].jointPosition[0] * 180/Math.PI;
            // Debug.Log(twist);
        ...
        public void UpdateJointTheta(int jointIndex, float target_theta)
        {
            JointControl joint = articulationChain[jointIndex].GetComponent<JointControl>();
            joint.controltype = control;
            if (control == ControlType.PositionControl)
            {
                ArticulationDrive drive = joint.joint.xDrive;
